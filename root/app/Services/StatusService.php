...
class StatusService
{
            ...
            if (isset($outputItem['content']) && is_array($outputItem['content'])) {
                foreach ($outputItem['content'] as $contentBlock) {
                    if (isset($contentBlock['type']) && $contentBlock['type'] === 'output_text' && isset($contentBlock['text'])) {
                        ...
                        $decodedContent = self::decodeStructuredContent(
                            $contentBlock['text'],
                            $accountName,
                            $accountOwner
                        );
    ...
    /**
     * Clean and decode JSON generated by the model, repairing truncated payloads where possible.
     *
     ...
     */
    private static function decodeStructuredContent(string $rawContent, string $accountName, string $accountOwner): array
    {
        // 1. Remove illegal control characters (but KEEP emojis)
        $cleaned = preg_replace('/[\x00-\x1F\x7F]/u', '', $rawContent);
        $cleaned = $cleaned ?? '';
        $trimmed = rtrim($cleaned);

        $candidates = [$trimmed];

        if (self::shouldAttemptJsonRepair($trimmed)) {
            $candidates[] = self::repairTruncatedJson($trimmed);
        }
        ...
        $error = "Error generating status for $accountName owned by $accountOwner: Invalid structured output from API (JSON decode error after clean-up: $jsonError). Original content: " . var_export($rawContent, true);
        ErrorManager::getInstance()->log($error, 'error');

        return ["error" => $error];
    }

    private static function shouldAttemptJsonRepair(string $content): bool
    {
        if ($content === '') {
            return false;
        }
