...
function shareButton(string $statusText, string $imagePath, string $accountOwner, string $accountName, int $statusId): string
{
    ...
    $filename = basename($imagePath); // Filename itself should be safe if generated by uniqid()
    // Construct the full image URL using rawurlencode for path segments
    // $filename is also encoded for consistency, though less critical if always simple.
    $imageUrl = DOMAIN . "/images/" . rawurlencode($accountOwner) . "/" . rawurlencode($accountName) . "/" . rawurlencode($filename);
    // Encode the status text for safe HTML output (used in data attributes)
